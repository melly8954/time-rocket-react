...
const GroupRocketCreate = () => {
  ...
  const isAllMembersComplete = () => {
    return members.length > 0 && members.every(member => member.status === 'COMPLETE');
  };
  ...
  const handleKickMember = async (memberId) => {
    ...
    if (window.confirm('정말로 이 멤버를 추방하시겠습니까?')) {
      try {
        ...
        await api.post(`/groups/${groupId}/rocket-chat`, systemMessage);
        fetchChatMessages();
      } catch (err) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      await api.post(`/groups/${groupId}/rocket-chat`, messageData);
      setNewMessage('');
      fetchChatMessages(); // 채팅 메시지 다시 조회
    } catch (err) {
      ...
      <div className={styles.mainLayout}>
        ...
        <div className={styles.leftSection}>
          ...
          <div className={styles.participantsCard}>
            <div className={styles.cardHeader}>
              <h3>👥 참가자 현황 ({members.filter(m => m.status === 'COMPLETE').length}/{members.length})</h3>
